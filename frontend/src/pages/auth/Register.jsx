...
import { fetchAllMentors, fetchAllIndustries, register } from "../../utils/actions";
...
export default () => {
	...
	const handleSubmit = async () => {
		...
		if (data.password.length < 7 || data.password_confirm.length < 7) {
		...
		await register(data).then((res) => {
			if (res.status == 200) {
				...
				setPageError(res.response ? res.response.data.error : res)
			}
		});
	}
			...
			<div className="grid h-screen max-w-lg grid-cols-1 mx-auto">
				<div className="bg-white shadow-md rounded px-8 pt-6 pb-8 my-auto">
					<div className="mb-5">
						...
						<div className="text-sm text-red-700 text-center mt-2">{pageError}</div>
					</div>
					<div className="">
						<div className="py-5">
							<div className="flex">
								<div className="my-4 pe-2 w-full">
									...
									<div className="flex items-center justify-end mb-3">
										<Link
											className="inline-block　align-baseline font-bold text-xs text-primary hover:text-blue-800"
											to={Paths.login}
										>
											すでに登録済みの方はこちら
										</Link>
